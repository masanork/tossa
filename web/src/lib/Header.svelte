...
<script lang="ts">
  ...
  import { Shield, Radio, Sparkles, MapPin, KeyRound, AlertTriangle, MessageSquareLock, Globe } from '@lucide/svelte';
  ...
  let { settings, user, onOpenAdmin, onOpenCreate, onOpenMessages }: Props = $props();
    ...
    <div class="flex items-center gap-1.5 sm:gap-2">
      ...
      <div class="relative">
        ...
        >
          ...
          <span class="text-xs">
            {LANGUAGES.find(l => l.code === i18n.current)?.shortLabel || 'Language'}
          </span>
        </button>
      ...
      <button
        ...
        title={user ? `${user.displayName}（クリックでメニュー表示）` : 'Passkey 認証・登録'}
      >
        {#if user}
          {#if user.role === 'admin'}
          ...
          {/if}
          <span class="font-semibold text-slate-800 truncate max-w-[120px]">{user.displayName}</span>
        {:else}
