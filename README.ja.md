...
2. **写真による簡単投稿 & EXIF GPS 自動ピン配置・位置微調整**
   - 現場写真をアップロードするだけで、EXIF メタデータから撮影地点の緯度経度を自動抽出し、地図上にピンを自動配置。
   - 地図ピッカーの拡大表示（`h-48` ⇄ `h-80`）、約10m単位の十字キー位置微調整（Nudge）、国土地理院APIによる逆引き住所の1タップ補完を完備。
...
5. **多言語・カラーユニバーサルデザイン (CUD) & ウェブアクセシビリティ対応 (JIS X 8341-3:2016 適合レベルAA / WCAG 2.1/2.2 AA)**
   - デジタル社会推進標準ガイドライン群（DS-600番台 / ウェブアクセシビリティ導入ガイドブック）およびデジタル庁デザインシステムに準拠。
   - **カラーユニバーサルデザイン (CUD)**: 色覚多様性（P型・D型等）や直射日光下、モノクロ画面に配慮し、ステータスバッジや地図ピンに色だけでなく明確な記号（✔/○ 利用可、▲ 混雑・残少、✖/✕ 終了・休止、? 確認中）と高コントラスト枠線を常時併記。
...
9. **PWA (Progressive Web App) & オフライン耐障害性（Outbox キュー管理）**
   - Service Worker によりアプリシェルおよび直近の投稿一覧を自動キャッシュ。
   - 通信途絶時でも現場投稿やステータス更新をローカル Outbox に一時保存し、電波復旧時に自動同期。
   - 未送信データの内訳確認、誤登録アイテムの個別破棄、送信失敗時の再試行フィードバックを完備。
...
1. 画面右上の **Passkey** を開く
2. お名前を入れて **Passkey で認証** を押す
3. 指紋または顔認証で完了（パスワードは不要）
4. 最初に認証した人が管理者になります
5. 認証後は、対象地域名や緊急アナウンスの設定ができます
