...
> 平時は地域の生活・店舗・助け合いを共有し、災害時はワンクリックで避難・給水・ライフライン情報板へと切り替わる、Cloudflareネイティブの超軽量デュアルユースCMS。
...
- **課題の解消と発展**:
  - **脱・固定スキーマ**: 平時（カフェ、イベント、地域掲示板）と有事（給水、炊き出し、断水）をコード改修なしでタグ・属性として自由に拡張可能にする。
...
2. **`posts`**:
   ...
   - **`attributes` (JSON)**: カテゴリごとに自由なタグ・詳細情報を持たせる
     - 有事例: `{"supplies": ["水", "オムツ"], "water_source": "well", "toilet_type": "temporary"}`
     - 平時例: `{"wifi": true, "parking": true, "hours": "10:00-18:00"}`
