// src/municipalities.ts: Municipality Code Master & Geocoding Resolver
export interface MunicipalityInfo {
  code: string; // 5-6 digit Local Government Code (全国地方公共団体コード)
 ...
 */
export const PREFECTURES: MunicipalityInfo[] = [
...
/**
 * Curated list of prominent municipalities, prefectural capitals, and representative disaster-risk areas
 * for offline-first instant autocomplete matching.
 */
export const POPULAR_MUNICIPALITIES: MunicipalityInfo[] = [
  // 北海道・東北
...
/**
 * Searches local municipality master, and falls back to GSI AddressSearch API
 * for exhaustive matching across all 1,741 municipalities in Japan.
 */
export async function searchMunicipalities(
  ...
  // 1. First search 47 prefectures whole-area master
  ...
  // 2. Search local curated municipalities
  ...
  // If we got enough local matches, return immediately
  if (combinedMatches.length >= 3) {
    return combinedMatches;
  }

  // 2. Query GSI AddressSearch API to support any municipality or town/village in Japan
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data)) {
        const remoteResults: MunicipalityInfo[] = [];
    ...
    console.warn('GSI geocoder search fallback error:', e);
  }
...
/**
 * Calculates zoom-10 tile coordinates (x, y) for a given lat/lng
 */
export function latLngToTileZ10(lat: number, lng: number): { z: number; x: number; y: number } {
  const z = 10;
